...
def resolve_package_sources(
    ...
    def get_pypi_files() -> list[dict]:
        ...
        if pypi_files is None:
            ...
            if known_hashes:
                ...
                if missing:
                    sys.exit(
                        f"ERROR: Hash(es) " + str(missing) + " for " + name + "==" + version + " "
    ...
    def policy_error(msg: str) -> NoReturn:
        sys.exit(f"ERROR: artifact-policy '" + str(policy) + "' for " + repr(name) + ": " + format_error(msg))
...
if not output_package:
    if opts.requirements_file:
        output_package = "python" + python_version + "-" + os.path.basename(opts.requirements_file).replace(".txt", "")
    elif len(packages) == 1:
...
for package in packages:
    if package.name is None:
        print(
            f"Warning: skipping invalid requirement specification " + str(package.line) + " "
